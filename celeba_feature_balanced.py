...
if __name__ == "__main__":
    ...
    data = CelebA(root='./data', split='all', target_type='attr', transform=CelebATransform(), download=True)
    ...
    with torch.no_grad():
        for images, attr in dataloader:
            ...
            male.append(attr[:, 20].cpu())
    all_latents = torch.cat(all_latents).numpy()
    labels = torch.cat(all_labels).numpy()
    male = torch.cat(male).numpy()
    is_male = male == 1

    for i, attr in enumerate(sorted_attrs):
        labels = all_labels[:, i]
        with_feature = labels == 1

        # balance feature distribution between male/female
        print(f'Balancing features for {attr}...')
        pos_male = all_latents[is_male & with_feature]
        pos_female = all_latents[~is_male & with_feature]
        neg_male = all_latents[is_male & ~with_feature]
        neg_female = all_latents[~is_male & ~with_feature]
        print(f'pos, male: {len(pos_male)}\npos, female: {len(pos_female)}\nneg, male: {len(neg_male)}\nneg, female: {len(neg_female)}')
        l = min(len(pos_male), len(pos_female), len(neg_male), len(neg_female))
        print(f'New length: {4*l}')
