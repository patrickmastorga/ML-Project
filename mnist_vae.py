...
class ConvEncoder(nn.Module):
    ...
    Maps input images to a latent space using a linear layer and ReLU activation.
    """
    def __init__(self, latent_dims=2, hidden_layer_size=128, initial_channels=16):
        """
        Args:
            latent_dims (int): Number of dimensions in the latent space.
            hidden_layer_size (int): Size of the hidden layer.
        ...
        super().__init__()
        self.network = nn.Sequential(
            nn.Conv2d(in_channels=1, out_channels=initial_channels, kernel_size=3, stride=2, padding=1),
            nn.ReLU(),
            nn.Conv2d(in_channels=initial_channels, out_channels=initial_channels*2, kernel_size=3, stride=2, padding=1),
            nn.ReLU(),
            nn.Flatten(),
            nn.Linear(in_features=initial_channels*2*7*7, out_features=hidden_layer_size),
            nn.ReLU()
        )
        self.output1 = nn.Linear(in_features=hidden_layer_size, out_features=latent_dims)
        self.output2 = nn.Linear(in_features=hidden_layer_size, out_features=latent_dims)
...
class ConvDecoder(nn.Module):
    ...
    def __init__(self, latent_dims=2, hidden_layer_size=128, initial_channels=16):
        ...
        self.network = nn.Sequential(
            nn.Linear(in_features=latent_dims, out_features=hidden_layer_size),
            nn.ReLU(),
            nn.Linear(in_features=hidden_layer_size, out_features=initial_channels*2*7*7),
            nn.ReLU(),
            nn.Unflatten(dim=1, unflattened_size=(initial_channels*2, 7, 7)),
            nn.ConvTranspose2d(in_channels=initial_channels*2, out_channels=initial_channels, kernel_size=3, stride=2, padding=1, output_padding=1),
            nn.ReLU(),
            nn.ConvTranspose2d(in_channels=initial_channels, out_channels=1, kernel_size=3, stride=2, padding=1, output_padding=1),
            nn.Sigmoid()
        )
